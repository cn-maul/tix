...
- 公开提单集成 `tianai-captcha-go` 滑块验证码，无需短信服务
...
- 无需登录即可提交工单
- 提交字段包含分类、问题描述、联系人、手机号和滑块验证码校验结果
- 提交成功后会返回工单编号、标题、分类与提示信息
- 公开工单会以 `public` 标签写入系统，方便后续筛选

## 滑块验证码

公开提单页面使用滑块拼图完成行为验证，不再依赖短信网关或测试验证码。

- 获取验证码接口：`GET /v1/public/captcha`
- 校验接口：`POST /v1/public/captcha/verify`
- 用户在公开提单页完成滑块验证后，方可提交工单
- 验证流程在页面内完成，无需短信服务
...
| GET | `/v1/public/captcha` | 获取滑块验证码 |
| POST | `/v1/public/captcha/verify` | 校验滑块验证码 |
