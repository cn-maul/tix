...
| 表单 | react-hook-form + zod | 轻量校验，类型安全 |
...
项目为单个 `main` 包，按职责拆分 5 个 Go 源文件 + 2 个测试文件，无子包：

| 文件 | 职责 | 规模 |
|---|---|---|
| `main.go` | 入口：参数解析、DB 打开/迁移、路由注册、SPA 回退、MIME 类型、鉴权中间件挂载 | ~251 行 |
| `store.go` | 数据模型（Ticket/Comment/Category/Stats/User）、建表、迁移、全部数据访问函数、统计、设置 KV | ~784 行 |
| `api.go` | JSON REST API 处理器（工单/备注/分类/统计/CSV）+ 字段校验 + 通用响应工具 | ~660 行 |
| `auth.go` | 内存会话表、登录/登出/状态、用户管理接口、设置接口（公开键白名单过滤） | ~355 行 |
| `notify.go` | 统一推送模块：消息模型、渠道抽象、PushPlus 渠道、推送配置管理端接口 | ~336 行 |
| `handlers_test.go` | JSON API 端到端测试 | ~550 行 |
| `notify_test.go` | 推送配置/渠道/测试端点测试（PushPlus 用本地 httptest 假服务） | ~264 行 |
...
/api/notify/config                 GET    推送配置查看（Token 脱敏，管理员）
/api/notify/config                 PUT    推送配置更新（部分更新，管理员）
...
/settings               设置（管理员，含子 Tab）
  ├─ /settings/general          通用
  ├─ /settings/notifications    消息推送（PushPlus 配置 + 测试发送）
  ├─ /settings/users            用户管理
  ├─ /settings/categories       分类管理
  └─ /settings/data             数据与备份（CSV 导出）
/submit                 报修提交页（独立纯净路由，无侧边栏）
```
...
`spaFallback` 先尝试从 embed 中读取请求路径对应的静态文件（命中则带 `Cache-Control: max-age=86400` 直出）；未命中则返回 `index.html`，交给前端路由处理（支持刷新深链）。
...
2. `categories` 表为空时写入固定五类种子（含颜色、排序）

旧库无需手工升级，重启即自动迁移。

### 6.6 SQLite 单连接

`db.SetMaxOpenConns(1)` 限制为单连接，避免偶发 `database is locked`。启用 WAL 日志模式与 5s busy_timeout，兼顾备份与并发读。单人/小团队使用无性能问题。

### 6.7 列表排序

列表按 `ORDER BY id [desc|asc]` 排序（新的在前/在后）。

### 6.8 归档 = 状态变更

标记已处理仅改 `status` 字段（0→1），数据不移动。列表通过 `WHERE status = ?` 过滤。
...
- **渠道抽象**：实现 `notifyChannel` 接口（`configured` + `send`）并在 `newNotifier()` 注册即可接入新渠道；当前已实现 PushPlus
- **配置存储**：复用 `settings` KV 表（键前缀 `notify_`），每次发送实时读取，改动即时生效、无需重启
- **敏感隔离**：`GET /api/settings` 为公开接口，按白名单（`publicSettingKeys`）过滤，`notify_*` 只经管理端 `/api/notify/config` 读写，Token 仅返回脱敏形式
...
- 多用户 / 角色 / 细粒度权限（仅单口令登录保护管理端，`/submit` 公开）
